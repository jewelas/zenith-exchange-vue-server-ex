<template>
  ...
  >
        ...
        <div class="top-menu-bar" ref="topMenuBar">
          ...
          >
            ...
            <Select
              ...
              @focusChange="timeSeletFocus"
            >
              <ul class="time-line_list a-4-bg" slot="options">
                <li
                  v-for="(item, index) in timeOptions"
                  :key="item.value"
                  class="time-line_item a-7-bg b-1-cl"
                  :class="{'b-4-cl a-14-bg': timeValue === item.value || timeHoverIndex === index}"
                  @mouseover='timeHoverIndex = index'
                  @mouseout='timeHoverIndex = null'
                  @click='timeLineChange(item)'
                >
                  {{item.label}}
                </li>
              </ul>
    ...
    <div class="top-tab a-6-bg">
      <span
        @click="switchChartType(1)"
        @mouseenter="enter(1)"
        @mouseleave="leave()"
        class="switch-chart-type"
        :class="{'b-1-cl a-4-bg':chartTypeShow(1) }"
      >
        <!-- {{$t('trade.dataType2')}} -->
      ...
      <span
        ...
        @mouseleave="leave()"
        class="switch-chart-type"
        :class="{'b-1-cl a-4-bg':chartTypeShow(2) }"
      >
        <!-- 深度图 -->
        {{$t('trade.dataType3')}}
      ...
      <span class="full-btn" @click="fullTv">
        <template v-if="!isfullTv">
          <span @mouseenter="fullTip = true" @mouseleave="fullTip = false">
            <svg class="icon icon-14 hover_hide" aria-hidden="true">
              <use xlink:href="#icon-v5_20"></use>
            </svg>
            <svg class="icon icon-14 hover_show" aria-hidden="true">
              <use xlink:href="#icon-v5_20_h"></use>
            </svg>
            <c-tipPop :show="fullTip" posX="right" :text="$t('trade.full')" />
          </span>
          <!-- <i class="hoverHide">{{ $t("trade.full") }}</i>
          <i class="b-1-cl hoverShow">{{ $t("trade.full") }}</i> -->
        </template>
        <template v-else>
          <span @mouseenter="outFullTip = true" @mouseleave="outFullTip = false">
            <svg class="icon icon-14 hover_hide" aria-hidden="true">
              <use xlink:href="#icon-v5_20_1"></use>
            </svg>
            <svg class="icon icon-14 hover_show" aria-hidden="true">
              <use xlink:href="#icon-v5_20_1_h"></use>
            </svg>
            <c-tipPop :show="outFullTip" posX="right" posY="bottom" :text="$t('trade.outFull')" />
          </span>
...
<script>
import mixin from 'BlockChain-ui/PC/common-mixin/modules/newTrade/tradingView/tradingView';
import 'BlockChain-ui/PC/common-mixin/modules/newTrade/tradingView/tradingView.styl';
